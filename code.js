...
// Run experiments
const inputSizes = [5, 6, 7, 8, 9, 10, 11, 12, 13, 15, 16, 17, 18];
const hkTimes = [], lsTimes = [];
const hkLengths = [], lsLengths = [];
...
for (const n of inputSizes) {
    ...
    if (n <= 18) {
        ...
        const hkLength = heldKarp(distMatrix);
        hkTimes.push((performance.now() - hkStart) / 1000);
        hkLengths.push(hkLength);
    } else {
        hkLengths.push(null); // Skipped
    }

    const lsStart = performance.now();
    const lsLength = localSearch(distMatrix);
    lsTimes.push((performance.now() - lsStart) / 1000);
    lsLengths.push(lsLength);
}
...
plot([
    { x: inputSizes, y: hkLengths, type: 'line', name: 'Held-Karp' },
    { x: inputSizes, y: lsLengths, type: 'line', name: 'Local Search' }
], {
    title: 'Tour Length vs Number of Cities',
    xaxis: { title: 'Number of Cities' },
    yaxis: { title: 'Tour Length' }
});
